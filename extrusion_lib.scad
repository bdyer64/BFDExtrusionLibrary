...
default_inside_miter = 0.8;
 ...
 module panel_rail(length,lip_width=default_lip_width,lip_height=default_lip_height, cap_height = default_cap_height,clips=[],clip_width = 10, clip_depth=5,outside_corner_miter_percent = default_outside_miter,inside_corner_miter_percent = default_inside_miter,handles=[],end_treat=END_TREAT_NONE) {
    ...
    rotate([-90,0,90])
        ...
        if(is_end_treat(end_treat,MITER_HORIZ_LEFT)) {
            translate([-10,-length/2,(lip_height+cap_height+1)/2]) rotate([0,90,0]) wedge([lip_height+cap_height+1,extrusion_width,extrusion_width]);
        }
        
        if (is_end_treat(end_treat,MITER_HORIZ_RIGHT)) {
            translate([-10,length/2,-(lip_height+cap_height+1)/2]) rotate([0,-90,180]) wedge([lip_height+cap_height+1,extrusion_width,extrusion_width]);
        }
 ...
 module corner_rail(corner_size,clips,end_connect = CORNER_NONE) {
    clip_offset_x = (corner_size[1] - extrusion_width)*(clips[1])+extrusion_width;
    clip_offset_y = (corner_size[0] - extrusion_width)*(1-clips[0]);
    ...
    union() {
        panel_rail(length=corner_size[0],clips = [clip_offset_y],clip_depth=5,handles=[HANDLE_LEFT],end_treat=MITER_HORIZ_LEFT+corner_end_x);
        translate([-corner_size[1]/2+extrusion_width/2,-corner_size[0]/2+extrusion_width/2,0]) rotate([0,0,-90])
        panel_rail(length=corner_size[1],clips = [clip_offset_x],clip_depth=5,handles=[HANDLE_RIGHT],end_treat=MITER_HORIZ_RIGHT+corner_end_y);   
       translate([4/2,-(corner_size[0])/2+default_lip_width,-(default_cap_height+default_lip_height)/2]) rotate([0,-90,0]) wedge([0.5,10,10]); 
    }
 }
 
 module corner_rail_pocket(corner_size,clips,end_connect = CORNER_NONE) {
    union() {
        panel_rail(length=corner_size[0],clips = [],clip_depth=5,lip_height=3.6,cap_height=4,outside_corner_miter_percent=0.7,inside_corner_miter_percent=0.8,end_treat=MITER_HORIZ_LEFT);
        translate([-corner_size[1]/2+extrusion_width/2,-corner_size[0]/2+extrusion_width/2,0]) rotate([0,0,-90])
        panel_rail(length=corner_size[1],clips = [],clip_depth=5,lip_height= 3.6,cap_height=4,outside_corner_miter_percent=0.7,inside_corner_miter_percent=0.8,end_treat=MITER_HORIZ_RIGHT);
        translate([4/2,-(corner_size[0])/2+12,-(4+3.6)/2]) rotate([0,-90,0]) wedge([0.5,10,10]);
    }
 }
...
//joint();
corner_rail(corner_size=[50,75],clips=[0.5,0.5]);//,end_connect=CORNER_MALE);
//clip_in_rail();
//rail_clip(10,5);
//rail_interior(10);
//rail_clip_hole(10,5);
